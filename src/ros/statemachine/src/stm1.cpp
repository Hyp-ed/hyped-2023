...
class Stm1TransitionSub : public rclcpp::Node {
 public:
  Stm1TransitionSub(std::unique_ptr<hyped::state_machine::StateMachine> &stm1,
                    std::shared_ptr<Stm1pub> &pub)
      : Node("Stm1TransitionSub"),
        stm1_(stm1),
        pub_(pub)
  {
    subscription_ = this->create_subscription<std_msgs::msg::String>(
      "stm1", 10, std::bind(&Stm1TransitionSub::topic_callback, this, _1));
  }

 private:
  void topic_callback(const std_msgs::msg::String &msg) const
  {
    RCLCPP_INFO(this->get_logger(), "Received Transition: '%s'", msg.data.c_str());
    hyped::state_machine::Message message = stm1_->stringToMessage(msg.data);
    bool transitioned                     = stm1_->handleMessage(message);
...
int main(int argc, char *argv[])
{
  ...
  std::shared_ptr sub2 = std::make_shared<Stm1TransitionSub>(stm1, pub1);

  rclcpp::executors::StaticSingleThreadedExecutor executor1;

  // executor.add_node(pub);
  executor1.add_node(sub1);
  executor1.add_node(sub2);
