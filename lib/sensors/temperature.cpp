...
std::optional<std::uint16_t> Temperature::read()
{
  if (i2c_.readByte(TEMPERATURE_1, STATUS) == 0) {
    auto Temp_High = i2c_.readByte(TEMPERATURE_1, TEMP_H);
    auto Temp_Low  = i2c_.readByte(TEMPERATURE_1, TEMP_L);
    // getting the high and low values from the register

    auto Temp = (TEMP_H << 8) | TEMP_L;
    Temp      = Temp * 0.01;

    return std::optional<std::uint16_t>{Temp};
...
{
  i2c_.writeByte(TEMPERATURE_1, CTRL, 1);
