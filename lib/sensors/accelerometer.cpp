...
Accelerometer::Accelerometer(std::uint8_t mux_address, io::I2c &i2c, core::ILogger &log) : ADDR(mux_address), log_(log), i2c_(i2c)
{
}
...
Accelerometer::~Accelerometer()
{
    ...
    std::optional<std::int16_t> getRawAccelerationX() {
		...
		if (!resultLow.hasValue() || !resultLow.hasValue()) {
			return std::nullopt;
		}
    ...
    std::optional<float> Accelerometer::read() 
    {
		...
		// todoLater: probably have to differentiate between the two tests. The reading failing completely might need to retry configuring the sensor
		if (!result.hasValue() || result.value() == 0x00) {
			return std::nullopt;
		}	
