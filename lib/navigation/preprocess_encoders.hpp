...
class EncodersPreprocessor {
 ...
 private:
  ...
  /** TODO
   * @brief changes the value corresponding to the encoder sensor in the encoders_reliable_ array to false if the number of consecutive 
   * outliers exceeds the threshold value
   * 
   * @return SensorChecks::kUnacceptable if more than 1 sensors have become unreliable and SensorChecks::kAcceptable otherwise
  */
   ...
   * @param quartile_percent value corresponding to the quantile that we want(0.25 for q1, 0.5 for median and 0.75 for q3)
   * @return the quantile that we require
  */
  template<std::size_t N>
  core::Float getSpecificQuantile(const std::array<std::uint32_t, N> &reliable_data, const core::Float quartile_percent){
