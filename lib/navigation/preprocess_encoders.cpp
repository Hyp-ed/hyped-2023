...
std::array<core::Float,3> EncodersPreprocessor::quartiles(T encoder_data){
  ...
  q1 = encoder_data.at(q1_low-1) + (((encoder_data.size()+1)/4.0) -
q1_low)*(encoder_data.at(q1_high-1) - encoder_data.at(q1_low-1)); q3 = encoder_data.at(q3_high-1)
+(((3*(encoder_data.size()+1))/4.0) - q3_low)*(encoder_data.at(q3_high-1) -
encoder_data.at(q3_low-1)); if(encoder_data.size() % 2 == 0){ median =
(encoder_data.at((encoder_data.size()/2)-1) + encoder_data.at(((encoder_data.size()/2) + 1))-1)/2.0;
  }
  else{
...
/*core::EncoderData EncodersPreprocessor::detectOutliers(const core::EncoderData encoder_data){
  const uint8_t num_reliable_encoders =
std::accumulate(are_encoders_reliable_.begin(),are_encoders_reliable_.end(),0); core::Float median;
  core::Float upper_bound;
  core::Float lower_bound;
  ...
  if(num_reliable_encoders == 3){
    ...
    std::array<core::Float,3> array_of_quartiles =
quartiles(encoder_data_copy_for_finding_quartiles); // ask how to pass the value median =
array_of_quartiles.at(0); upper_bound = array_of_quartiles.at(1); lower_bound =
array_of_quartiles.at(2);
  }
  else{
...
are_encoders_reliable_.at(i) == false){ encoder_data_copy.at(i) = median;
...
core::EncoderData EncodersPreprocessor::detectOutliers(const core::EncoderData encoder_data)
{
  ...
  if (num_reliable_encoders_ == core::kNumEncoders - 1) {
    std::array<uint32_t, core::kNumEncoders - 1> encoder_data_copy_for_finding_quartiles;
    ...
    std::sort(encoder_data_copy_for_finding_quartiles.begin(),
              encoder_data_copy_for_finding_quartiles.end());
    quartiles = getQuartiles(encoder_data_copy_for_finding_quartiles);  // ask how to pass the value
  } else {
    core::EncoderData encoder_data_copy_for_finding_quartiles;
    std::copy(
      encoder_data.begin(), encoder_data.end(), encoder_data_copy_for_finding_quartiles.begin());
    std::sort(encoder_data_copy_for_finding_quartiles.begin(),
              encoder_data_copy_for_finding_quartiles.end());
    quartiles = getQuartiles(encoder_data_copy_for_finding_quartiles);  // ask how to pass the value
  }
