...
void I2c::setSensorAddress(const std::uint8_t device_address)
{
  ...
  if (return_value < 0) {
    log_.log(hyped::core::LogLevel::kFatal, "Could not set sensor address");
    return;
  }
}
                                          ...
                                          const std::uint8_t register_address)
{
  ...
  if (num_bytes_read != 1) {
    log_.log(hyped::core::LogLevel::kFatal, "Could not read from i2c device");
    return std::nullopt;
  }
  return read_buffer[0];
}

hyped::core::Result I2c::writeByteToRegister(const std::uint8_t device_address,
                                             const std::uint8_t register_address,
...
hyped::core::Result I2c::writeByte(const std::uint8_t device_address, const std::uint8_t data)
{
  if (file_descriptor_ < 0) {
    log_.log(hyped::core::LogLevel::kFatal, "Could not find i2c device while writing");
    return hyped::core::Result::kFailure;
