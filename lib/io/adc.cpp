...
Adc::Adc(const std::uint8_t pin, core::ILogger &logger) : pin_(pin), logger_(logger)
{
  char buf[100];
...
std::optional<std::uint16_t> Adc::readValue()
{
  ...
  if (raw_voltage) {
    logger_.log(core::LogLevel::kDebug, "Raw voltage: %i", raw_voltage.value());
    return *raw_voltage;
  }
  return std::nullopt;
}
...
std::optional<std::uint16_t> Adc::resetAndRead4(const int file_descriptor)
{
  ...
  if (num_bytes_read != sizeof(read_buffer)) {
    logger_.log(core::LogLevel::kFatal, "Failed to read 4 bytes");
