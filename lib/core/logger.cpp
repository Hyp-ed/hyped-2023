...
void Logger::printHead(FILE *file, const char *title)
{
  ...
  const std::chrono::milliseconds time_point_milliseconds = duration_cast<std::chrono::milliseconds>(time_point - time_point_seconds);
  const std::tm *time_struct = localtime(&ttime_t);
  fprintf(file, "%02d:%02d:%02d.%03lld %s[%s] ", time_struct->tm_hour, time_struct->tm_min, time_struct->tm_sec, time_point_milliseconds.count(), title, label_);
};
